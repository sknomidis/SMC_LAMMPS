...
# Dimensions of simulation box (nm)
boxDx = DNAcontourLength + 2*armLength
boxDy = DNAcontourLength/2 * (1-relExtension**2)**0.5 + armLength
boxDz = DNAcontourLength   * (1-relExtension**2)**0.5 + 2*armLength
...
theta = opt.fsolve(lambda theta,x: np.sin(theta/2)-theta/2*x, 
                                   np.pi, args=(relExtension,))[0]

# Radius of circular arc
radius = DNAcontourLength/theta

thetas = np.linspace( (theta+np.pi)/2, -(theta-np.pi)/2, nDNA )

rDNA      = np.zeros((nDNA,3))
rDNA[:,0] = radius * np.cos(thetas)
rDNA[:,1] = radius * np.sin(thetas)

# Shift ends to y=0
rDNA[:,1] -= np.amin(rDNA[:,1])
...
# DNA tangent vector
u  = rDNA[attachment+1] - rDNA[attachment-1]
u /= np.linalg.norm(u)

# Axis of lower binding site
v  = rSiteD[1] - rSiteD[0]
v /= np.linalg.norm(v)
...
rArmUR = np.einsum('ij,...j->...i', rotMat, rArmUR)
rArmDR = np.einsum('ij,...j->...i', rotMat, rArmDR)
rHK    = np.einsum('ij,...j->...i', rotMat, rHK)
rATP   = np.einsum('ij,...j->...i', rotMat, rATP)
rSiteU = np.einsum('ij,...j->...i', rotMat, rSiteU)
rSiteM = np.einsum('ij,...j->...i', rotMat, rSiteM)
rSiteD = np.einsum('ij,...j->...i', rotMat, rSiteD)
...
# Axis of lower compartment
w  = rSiteM[1] - rSiteD[1]
w /= np.linalg.norm(w)

# Makes sure that the SMC encircles DNA at the right position
shift = rDNA[attachment] - rSiteD[1] - w * sigmaSiteDvsDNA * 2**(1/6)
...
# Minimum of belt bond
#bondMinBelt = intRadSMCvsDNA
bondMinBelt = 0
...
datafile.write("5 harmonic %s %s\n\n"           %(kBondBelt, bondMinBelt))
...
datafile.write("%s 5 %s %s\n\n" %(nDNA+14,  IDhK[nHK//2], IDdNA[tether]))
...
"""
plt.plot(  rDNA[:,0],   rDNA[:,1], '-')
plt.plot(   rHK[:,0],    rHK[:,1], '.')
plt.plot(  rATP[:,0],   rATP[:,1], '.')
plt.plot(rArmDL[:,0], rArmDL[:,1], '.')
plt.plot(rArmUL[:,0], rArmUL[:,1], '.')
plt.plot(rArmUR[:,0], rArmUR[:,1], '.')
plt.plot(rArmDR[:,0], rArmDR[:,1], '.')
plt.plot(rSiteU[0,0], rSiteU[0,1], '.')
plt.plot(rSiteD[0,0], rSiteD[0,1], '.')
